...
const Filters: FC<TFilters> = ({ onChangeFilter }) => {
  ...
  return (
    <Container justify={'flex-start'} style={{
      ...
      <Ionicons onPress={() => onChangeFilter('place')} style={{
        marginLeft: 20
      }} name="location" size={34} color={currentFilter === 'place' ? '#FA4E74' : 'white' } />
      <Ionicons onPress={() => onChangeFilter('calendar')} style={{
        marginLeft: 20
      }} name="calendar-number" size={34} color={currentFilter === 'calendar' ? '#FA4E74' : 'white' } />
      <Ionicons style={{
        marginLeft: 20
